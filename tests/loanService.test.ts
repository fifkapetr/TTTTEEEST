...
describe("loanService", () => {
  ...
  describe("deleteLoan", () => {
    it("removes a loan by id and persists the updated list", () => {
      const firstLoan: LoanApplication = {
        id: "loan-1",
        applicantName: "Alice",
        amount: 50000,
        termMonths: 24,
      ...
      const secondLoan: LoanApplication = {
        id: "loan-2",
        applicantName: "Bob",
        amount: 60000,
        termMonths: 36,
      ...
      saveLoans([firstLoan, secondLoan]);

      deleteLoan("loan-1");

      expect(getLoans()).toEqual([secondLoan]);
    });

    it("throws error for non-existent loan", () => {
      expect(() => deleteLoan("missing-id")).toThrow(
        "Loan with id missing-id not found",
      );
    });
  });
